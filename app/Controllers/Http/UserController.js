...
class UserController {
  ...
  async demoAdmin({auth, request, response}) {
    ...
    try {
      ...
      if (auth.user) {
        await auth.logout()
        return response.send({type:"info", msg:"You are already in HealthyVerse, please log in on the top right button"})
      }else{
        await User.create({wallet: wallet.toLowerCase(), nonce, admin: true})
    ...
    try {
      ...
      if (!user) {          
       ...
       if (!query[0]) {
        ...
        msg = "Welcome to HealthyVerse. New admin user created"
        type = "info"

      }else{
        msg = "Sorry, only registered users can access to HealthyVerse"
        type = "error"
      }
